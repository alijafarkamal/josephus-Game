...
void drawJosephusCircle(SDL_Renderer* renderer, TTF_Font* font, const vector<int>& order, int currentStep) {
    ...
    // Assume people are positioned in a circle initially numbered 1 to n
    int n = order.size();
    for (int i = 1; i <= n; i++) {
        ...
        SDL_Color color;
        if (currentStep < order.size() && i == order[currentStep]) {
            color = {255, 0, 0, 255}; // Red for eliminated person
        } else {
            color = {255, 255, 255, 255}; // White for others
        }
