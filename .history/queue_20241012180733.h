...
void drawJosephusCircle(SDL_Renderer* renderer, TTF_Font* font, const vector<int>& order, int currentStep) {
    ...
    int n = order.size(); // Total number of people
    float radius = 250.0;
    SDL_Point center = {WINDOW_WIDTH / 2, WINDOW_HEIGHT / 2};
    static vector<bool> eliminated(n, false); // Track eliminated positions, persist state across frames

    // Update the eliminated state for current step
    if (currentStep < order.size()) {
        eliminated[order[currentStep]] = true;  // Mark current elimination
    }

    // Draw all positions with eliminated ones in red
    for (int i = 0; i < n; i++) {
        double angle = 2 * M_PI * i / n;
        int x = center.x + int(radius * cos(angle)) - 10; // Adjust text position
        int y = center.y + int(radius * sin(angle)) - 10;
